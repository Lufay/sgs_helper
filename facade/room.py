...
def make_room(cmd, ctx, *args, **kwargs):
    ...
    res = send_card(kwargs.get('chat_id'), 'ctp_AAy5FDrz9DNP', room_id=room.room_id)
    print(res)
    return room.room_id
...
def get_role(cmd, ctx, *args, **kwargs):
    ...
    room = Room(room_id)
    return simple_card(room_id, room.pop_role(op_open_id).value)


# @route('get_position', MT.PREFIX)
def get_position(cmd, ctx, *args, **kwargs):
    raise DeprecationWarning('Room.rooms_map has been deprecated')
    room_id = ctx[0].strip()
